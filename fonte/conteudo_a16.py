...
AULA = {
 ...
 "passos": [
  ...
  dict(n=16, titulo="Guarde o RECORDE, não só o último", img="aula16/p_recorde_pronto.jpg", clipe=None,
    corpo="Mostrar o último tempo é pouco: o que faz jogar de novo é o <b>melhor</b> tempo.<br><br>No script da <span class=ui>Chegada</span>, troque a linha que escreve no placar por estas <b>três</b>:<br><br><span class=ui>local antes = tonumber(string.match(jogador.leaderstats.Tempo.Value, \"%d+%.?%d*\"))</span><br><span class=ui>if not antes or segundos &lt; antes then</span><br><span class=ui>jogador.leaderstats.Tempo.Value = segundos .. \" s\"</span><br><br>e feche com um <span class=ui>end</span> a mais depois delas.<br><br>Traduzindo: <i>pegue o número que está no placar; se não houver nenhum, ou se o de agora for menor, escreva o de agora</i>.",
    ck="O placar só muda quando você faz um tempo <b>melhor</b> que o anterior.",
    sos=[("Deu erro no string.match","É o caminho mais chato da aula. Se travar, use a versão simples: guarde o recorde num bilhete, com <span class=ui>jogador:SetAttribute(\"Recorde\", segundos)</span>, e compare com ele."),
         ("Sobraram end a mais","Você acrescentou um <span class=ui>if</span>, então precisa de <b>um</b> end a mais — não dois."),
         ("O placar nunca muda","O primeiro tempo tem de entrar: confira se o <span class=ui>not antes</span> está na condição.")]),
